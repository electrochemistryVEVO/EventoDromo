// src/components/carrito/TablaEntradas.view.tsx
"use client";

import React, { useRef, useEffect } from "react";
import { FilaEntrada, type CartItem } from "./filaEntrada";

type TablaEntradasViewProps = {
    items: CartItem[];
...
export const TablaEntradasView: React.FC<TablaEntradasViewProps> = ({
    items,
    selectedIds,
    isAllSelected,
    isIndeterminate,
    ...
    const renderContent = () => {
        if (isLoading) return <div className="p-4 text-center">Cargando entradas...</div>;
        if (error) return <div className="p-4 text-center text-red-500">{error}</div>;
        if (items.length === 0) return <div className="p-4 text-center">No hay entradas en tu carrito.</div>;
