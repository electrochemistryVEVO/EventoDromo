// src/components/carrito/ResumenCompra.view.js
"use client";
...
export const ResumenCompraView = ({ items, total, isLoading, error }) => {
  const renderContent = () => {
    if (isLoading) {
    ...
    if (items.length === 0) {
      return (
        <li className="p-4 text-center">No hay entradas en el carrito.</li>
      );
    }
    return items.map((item) => <FilaResumenCompra key={item.id} item={item} />);
  };
